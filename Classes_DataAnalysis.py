...
class DiscoAnalysis(DataAnalysis):
	...
	def plt_dev_pd(self):
		...
		df = pd.DataFrame((raw['actual_close_price'] - raw['pre_midquote']) / raw['pre_midquote'] * 10000,
					   columns=['dev'])
		...
		plt.tight_layout()
		plt.savefig(figdir + "\\PriceDiscovery\\Closing_Deviations_terciles.png", dpi=dpi)
		plt.show()
		plt.close()
